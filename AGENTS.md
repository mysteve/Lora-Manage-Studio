...
- 输出图片看图区域保持透明悬停背景；未放大时滚轮上下切图，同方向最短间隔 50ms、反向及时响应；相邻图片预解码最多保留 6 张，关闭看图或更换目录释放引用，卡片列表不随当前图片切换重复生成；放大后滚轮以鼠标位置为中心在适应窗口的 1–8 倍之间缩放。滚轮缩回适应窗口后短暂抑制切图，参数面板保留自身滚动。
