...
- 主导航：我的模型、在线发现、下载中心、我的图像、词句组合。设置使用内嵌页面，内部按工作空间、网站访问、AI 接入分为三个标签页，各自保存，切换标签保留草稿；详情由页面状态切换；当前没有引入前端路由库。
...
| `src/features/models/` | 模型列表按游标无限追加，每批最多 60 条，搜索、基础模型、文件状态或排序改变时重置列表；底部提供加载状态、失败重试及末尾提示，滚动后提供浮动回顶；模型导入、详情、资料编辑和预览图；详情默认显示官方触发词，支持点击单词及复制全部官方词，保留模型信息、下载和 `ImageGallery` 示例图及生成参数；不提供组合预览、个人配方编辑或自定义触发词展示 |
...
| `src/features/prompts/` | 提示词片段命名、就地编辑、启停、拖动与键盘排序及正负向分组组合；仅通过顶部按钮添加片段；编辑区高度限制在窗口内，仅片段列表内部滚动，添加按钮和底部操作保持可见；草稿由 App 通过 `usePromptDraft` 管理，切换页面保留，并通过独立的版本化 localStorage 自动保存及恢复；读取失败禁止覆盖，存储错误在编辑页显示，未保存修改参与关闭保护；命名预设通过 localStorage 持久化，使用版本化数据和独立 preview 键，读取失败时禁止覆盖 |
| `src/features/settings/` | ComfyUI 目录、代理、API 密钥及 AI 接入设置 |
| `src/features/outputs/` | 输出结果图片列表按游标无限追加，每批最多 60 张，以修改时间降序和相对文件名升序为游标边界，刷新或更换目录重置；底部提供加载状态、失败重试及末尾提示，滚动后提供浮动回顶，不显示翻页或任意跳页入口；OutputViewer 铺满应用窗口，方向键和两侧按钮支持跨批次切图，Esc 返回，生成参数按需展开；图片点击放大两倍、拖动查看、再次点击还原，切图重置缩放；接近已加载边界预取下一批，等待时保留当前图片，刷新或更换目录清除缓存；默认读取 ComfyUI 的 output，可在工作空间设置中自定义查看目录。按记录中的 LoRA 文件路径关联模型库，同名候选全部展示；打开库详情后返回恢复图片、已加载列表和滚动位置 |
| `src-tauri/src/services/outputs.rs` | 输出目录校验、递归图片扫描及打开目录；仅为实际返回批次的图片逐文件增加本地资源协议授权，不跟随符号链接；刷新或更换目录重置前端缓存，当前后端不撤销既有逐文件授权 |
...
- 模型无限追加预览使用 `?preview&library-pagination-preview`，仅此参数增加超过 60 条的明确示例，保留原有示例；图像使用 `?preview&outputs-preview` 的 65 张示例，两者可同时启用。
- `list_library_page({query, baseModel, fileStatus, sort, cursor})` 固定最多返回 60 条及 `{items, nextCursor, total}`，`total` 为筛选后数量；文件状态使用空字符串、`installed`、`missing`，排序使用 `newest`（createdAt 降序）、`size`（size 降序）、`name`（Rust 字符串升序），同值均按 id 升序；keyset 游标绑定搜索、筛选与排序条件，非法或条件不匹配时返回错误。后端在后台线程读取存储后筛选排序，不宣称数据库增量查询。追加失败保留已有内容并重试相同游标，旧条件响应不得写回新列表。
...
- 设置页通过 `onDirtyChange` 汇总工作空间、网站访问、AI 配置及未保存密钥输入，离开和关闭应用时保护修改；AI 初始化失败提供原地重试。
- 模型详情不加载或保存配方，不再产生配方 dirty 状态；保留 `Detail` 的兼容 props，不改其他功能中的配方能力。编辑本地资料不提供自定义触发词编辑，`update_entry` 必须传回 `entry.triggerWords` 原值；不得因移除界面清空既有触发词、组合预览或配方数据，也不删除后端、开发预览和存储支持。
- 在线发现的搜索输入草稿与已提交条件分离，游标绑定已提交条件，失败重试复用完整请求；详情请求在导航、关闭或更换选择后失效。
- 加入下载队列后保留详情，提供查看下载入口；失败任务提供计数和筛选，失败状态转换通知去重，进行中空列表需提示已有失败任务。
...
- 输出图片看图区域保持透明悬停背景；未放大时滚轮上下切图，同方向最短间隔 50ms、反向及时响应；相邻图片预解码最多保留 6 张，关闭看图或更换目录释放引用，卡片列表不随当前图片切换重复生成；放大后滚轮以鼠标位置为中心在适应窗口的 1–8 倍之间缩放。滚轮缩回适应窗口后短暂抑制切图，参数面板保留自身滚动。最上层看图窗口中，Shift + 方向键平移、+ / = 放大、- 缩小、0 复位；普通左右方向键仍翻图，输入控件及输入法组合期间不抢快捷键。
...
`src/features/prompts/terms.ts` 维护内置双语词库、搜索与光标位置补全逻辑；`PromptInput` 提供中英文及别名匹配、方向键选择和 Enter 补全，只有主动用方向键选择候选后 Tab 才补全，否则正常移动焦点；中文输入法组合期间不处理选择快捷键。补全保留当前词句之外的内容及权重语法，不搜索 LoRA 表达式内部。`PromptLibrary` 通过编辑区底部的词库管理入口维护英文词句、中文翻译、分类、别名和正负向范围，支持新增、编辑、启停和删除。词库通过版本化 localStorage 单独持久化，preview 使用独立键；写入前读取最新数据，读取失败禁止覆盖。
