                ...
                {(page === 'library' || page === 'favorites') && (
                  <>
                    ...
                    <button className="primary" disabled={!!scan?.running} onClick={startScan}>
                      {scan?.running ? <Loader2 size={17} className="spin" /> : <FolderSearch size={17} />}
                      扫描本地模型
                    </button>
                  </>
                )}
            ...
            {scan && (
              <div className={`scan-strip ${scan.running ? 'running' : ''}`}>
                <span>
                  ...
                  {scan.running ? '扫描中' : '扫描完成'} · {scan.processed} 个文件 · 已补齐 {scan.matched} 个
                </span>
                    ...
                    return (
                      <article className="download-row" key={t.id}>
                        ...
                        <div className="download-actions">
                          {['queued', 'downloading', 'verifying'].includes(t.status) && (
                            <button
                              className="icon-button"
                              aria-label={`暂停 ${t.model.name}`}
                              onClick={() =>
                                perform(() => call('control_download', { id: t.id, action: 'pause' }))
                              }
                            >
                              <Pause size={19} />
                            </button>
                          ...
                          {['paused', 'failed'].includes(t.status) && (
                            <button
                              aria-label={`继续 ${t.model.name}`}
                              onClick={() =>
                                perform(() =>
                                  call('control_download', {
                                    id: t.id,
                                    action: t.status === 'failed' ? 'retry' : 'resume',
                                  }),
                                )
                              }
                            >
                              {t.status === 'failed' ? <RefreshCw size={17} /> : <Play size={17} />}{' '}
                              {t.status === 'failed' ? '重试' : '继续'}
                            </button>
                          )}
                          {t.status === 'completed' ? (
                            <button
            ...
            {page === 'recipes' && (
              <>
                ...
                <div className="recipe-grid">
                  {recipes
                    .filter((r) =>
                      [r.name, r.positive, r.negative].join(' ').toLowerCase().includes(query.toLowerCase()),
                    )
                    .map((r) => {
